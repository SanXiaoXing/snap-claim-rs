...
pub fn build_preview_rows(results: &[InvoiceRecord], days: u32) -> Vec<PreviewRow> {
    ...
    let car_data: Vec<_> = results
        .iter()
        .filter(|r| r.kind == "car" && !r.is_round_trip)
        .collect();
    let round_trip_data: Vec<_> = results
        .iter()
        .filter(|r| r.kind == "car" && r.is_round_trip)
        .collect();
...
mod tests {
    ...
    fn preview_has_round_trip_column_after_city_transport() {
        ...
        let rt = rows
            .iter()
            .find(|r| r.cells.first().and_then(|c| c.as_str()) == Some("往返交通"));
        ...
        let city = rows
            .iter()
            .find(|r| r.cells.first().and_then(|c| c.as_str()) == Some("市内交通"));
